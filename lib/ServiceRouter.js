...
ServiceRouter.prototype.forwardRequestToService = function () {
  ...
  return function ( req, res, args, act, respond ) {
    ...
    act(args, function( err, out ) {
      ...
      out.httpstatus$ = out.ok ? Http.OK : reasonToHttpCode(out.why);
...
//    this._adminLocal(req, req.seneca.user)
    ...
    if( redirect ) {
      ...
      sendjson(reasonToHttpCode(out.why), res, out)
    }
  }

}

function reasonToHttpCode(why) {
  return why === 'restricted' ? Http.FORBIDDEN : 
    (why === 'user-not-found' ? Http.UNAUTHORIZED : Http.BAD_REQUEST);
}
...
ServiceRouter.prototype._respond = function(err, res, redirect, data) {
  ...
  if( redirect ) {
    ...
    if(_.isNumber(err)) {
      ...
      httpCode = reasonToHttpCode(data.why)
    }
